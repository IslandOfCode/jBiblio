...
public class EditClient extends JFrame {
	...
	public void setMode(CLIENTMODE mode, int ID, GUI parent) {
		this.MODE = mode;
		this.parent = parent;
			...
			public void actionPerformed(ActionEvent e) {
				if(MODE==CLIENTMODE.ADD) {
					...
					if(!client.equals(clientUpdate)) {
						...
						JOptionPane.showMessageDialog(contentPane, "Cliente aggiornato", "Aggiornamento", JOptionPane.INFORMATION_MESSAGE);
						Logger.info("Cliente ["+client.getID()+"], aggiornato");
						dispose();
					} //else do nothing
					Logger.warn("nessun aggiornamento effetturato ["+client.hashCode()+"]=["+clientUpdate.hashCode()+"]");
				}
				parent.signalFrameClosed(getTitle());
			}
		});
	...
	private Client retriveDataFromForm() {
		...
		return new Client(
				...
				this.TXT_cognome.getText(),
				Integer.parseInt((String) CB_classe.getSelectedItem()),
				(String) CB_sezione.getSelectedItem(),
				removed
				);
	}
