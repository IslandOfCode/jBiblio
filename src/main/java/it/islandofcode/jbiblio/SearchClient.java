...
public class SearchClient extends JFrame {
	...
	public static enum AFTERSEARCH{
		...
		EDIT,		//1 mostra un pulsante che porta alla schermata di modifica del cliente selezionato
		DELETE,		//2 mostra un pulsante che rimuove il cliente selezionato (dopo doppia conferma)
	...
	public SearchClient() {
		...
		B_searchClient.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(TXT_nome.getText().isEmpty() && TXT_cognome.getText().isEmpty() && CB_classe.getSelectedIndex()==0 && CB_sezione.getSelectedIndex()==0) {
					int r = JOptionPane.showConfirmDialog(rootPane, "Verranno mostrati TUTTI i clienti del database, vuoi continuare?", "Ricerca senza parametri", JOptionPane.YES_NO_OPTION);
					if(r!=0) return;
					...
					resultTable = new JTable(
							DBManager.searchClientsAsTableModel(null,null,-1,null,true)
							);
					...
					resultTable = new JTable(
							DBManager.searchClientsAsTableModel(
									TXT_nome.getText().trim(),
									TXT_cognome.getText().trim(),
									classe,
									tmp.equals("-")?null:tmp,
									false
									)
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(resultTable.getSelectedRow()<0) {
					...
					if(DBManager.getClientByID(ID).isRemoved()) {
						JOptionPane.showMessageDialog(contentPane, "Il cliente scelto non è più disponibile!", "Attenzione!", JOptionPane.WARNING_MESSAGE);
						return;
					}
					
					EditClient C = new EditClient();
					C.setMode(EditClient.CLIENTMODE.EDIT, ID, parent);
				}
			}
		});
		...
		B_createLoan.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(resultTable.getSelectedRow()<0) {
					...
					if(C.isRemoved()) {
						JOptionPane.showMessageDialog(contentPane, "Il cliente scelto è stato rimosso!", "Attenzione!", JOptionPane.WARNING_MESSAGE);
						return;
					}
		...
		B_removeSelected.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(resultTable.getSelectedRow()<0) {
					...
					if(C.isRemoved()) {
						JOptionPane.showMessageDialog(contentPane, "Il cliente scelto è già stato rimosso!", "Attenzione!", JOptionPane.WARNING_MESSAGE);
						return;
					}
					...
					if(ret<0) {
						JOptionPane.showMessageDialog(contentPane, "Non è stato possibile rimuovere il cliente.", "Errore DataBase!", JOptionPane.ERROR_MESSAGE);
						return;
					}
					
					JOptionPane.showMessageDialog(contentPane, "Il cliente scelto è stato rimosso!", "Operazione completata", JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
